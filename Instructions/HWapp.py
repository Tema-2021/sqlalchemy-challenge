...
def start(start, end):
    ...
    tobs_start_end = session.query(Measurement.date, func.min(Measurement.tobs),func.avg(Measurement.tobs), func.max(Measurement.tobs)).\
       filter(Measurement.date >= '2016, 9, 31').\
       filter(Measurement.date <= '2017, 9, 31').\
    ...
    # Convert list of tuples into a normal list
    start_end_tobs = list(np.ravel(tobs_start_end))
...
# refer to app.py for "api/v1.0/<start>" analysis